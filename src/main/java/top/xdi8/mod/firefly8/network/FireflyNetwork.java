...
public class FireflyNetwork {
    public static final PlayChannel CHANNEL = PlayChannel.create(
            new ResourceLocation("firefly8", "main"), 4, 0);
    ...
    static {
        ...
        CHANNEL.registerS2C(S2C_PREPARE_RESPAWN, env -> {
            env.enqueueWork(() -> {
                final Minecraft minecraft = Minecraft.getInstance();
                final LocalPlayer player = minecraft.player;
                if (player != null && minecraft.level != null) {
        ...
        CHANNEL.registerC2S(C2S_RESPAWN, env -> {
            env.enqueueWork(() -> env.serverPlayer().ifPresent(oldPlayer -> {
