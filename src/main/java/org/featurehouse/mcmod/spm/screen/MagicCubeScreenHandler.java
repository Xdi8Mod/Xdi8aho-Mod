...
public class MagicCubeScreenHandler extends AbstractContainerMenu {
    ...
    public MagicCubeScreenHandler(int syncId, Inventory inventory) {
        this(syncId, inventory, inventory.player.level, new SimpleContainer(8), new IntMagicCubeProperties.Impl());
    }

    public MagicCubeScreenHandler(int syncId, Inventory playerInventory, Level world, Container inventory, IntMagicCubeProperties properties) {
        super(SPMMain.MAGIC_CUBE_SCREEN_HANDLER_TYPE.get(), syncId);
        ...
        mainFuelSlot = this.addSlot(new FuelSlot(PEEL.get(), inventory, 6, 12, 32));
        viceFuelSlot = this.addSlot(new FuelSlot(POTATO_POWDER.get(), inventory, 7, 139, 30));
